...
from ssvae.vae_utils import plot_reconstruction,shuffle_X_y,plot_loss


class SupervisedVAE:

    def __init__(self,feature_size,model_type, attr_list,attr_sizes):
        self.model_type=model_type
        self.feature_size=feature_size

        self.attributes=OrderedDict()
        for attr,size in zip(attr_list,attr_sizes):
            self.attributes[attr]=size


        self.create_model(self.feature_size,self.model_type,self.attributes)
        self.start_session()
    ...
    def index_to_z_softmax(self,z):
        return np.concatenate([to_categorical(_z,num_classes=list(self.attributes.values())[i]) for i,_z in enumerate(z)])
    def z_to_X(self,z):
        ...
        return self.sess.run(self.output_layer,feed_dict={self.z:[z]}).reshape(self.feature_size,self.feature_size)

    def X_to_z(self,X):
        if(self.model_type=="softmax"):
            z=self.sess.run(self.z,feed_dict={self.X:[X]})[0]
            attr_values=np.array(list(self.attributes.values()))
            return np.stack([np.argmax(z[np.sum(attr_values[:i]):np.sum(attr_values[:i])+val]) for i,val in enumerate(attr_values)])
